...
int main(int argc, char** argv)
{
  ...
  TFile *f1 = new TFile(Form("/w/hallb-scifs1a/primex/liye/data/li_MC_pair64877.root",run1),"r");
   if (!f1) cout << " erro " << endl;
...
for(int i=0;i<18;i++) 
    {
      sprintf(hName,"pair_Ecounter%02d",i+1);
      sprintf(hTitle,"Ecounter%02d",i+1);
      pair_Ecounter[i] = new TH1F(hName,hTitle,300,-1.5,1.5);  
    }
  ...
  for(int i=1;i<=nevents;i++){
    ...
    if(idy <18) pair_Ecounter[idy]->Fill(e0);
 ...
 TFile *f = new TFile("/w/hallb-scifs1a/primex/liye/222/MCpair64877_e1_5_z200.root","RECREATE");
  h_z_e0->Write();
  h_z_e0_cut->Write();
