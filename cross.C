...
void fit_his(TH1F *h_data,TH1F *h_empty, TH1F *h_simulation, TH1F *h_outTcut, TH1F *h_pair, int i )
{
  ...
  fit_func->SetParLimits(2, 0.70, 0.74);    // p2 is for simulation// 
  ...
  h_data->Fit(Form("fit_func%d",i),"","",-0.5, 0.7);
...

{
  ...
  */
  ...
  for (int j=NEcounter_min-1 ; j<NEcounter_max ; j++)
    {
      ...
      cout << " Dyield = " << Dyield[j] << "---"<< "MCyield = " << MCyield[j]<< " Acc=" << Acc[j] <<" ****input_Bin =  "<< input_Bin[j]<< "flux_bin="<<flux_Bin[j]<<" j=" <<j<<endl;
  ...
  for ( int i=NEcounter_min-1 ; i< NEcounter_max ;i++)
    ...
    Ecross[i]= input_Bin[i]/flux_Bin[i];
    ratio[i] = Dyield[i]/flux_Bin[i];
  ...
  */
  ...
  TMultiGraph *mg = new TMultiGraph("mg","Yield/flux");
  TGraphErrors *g1 = new TGraphErrors(18,Energy,ratio,cross_error,0);
  TGraphErrors *g2 = new TGraphErrors(18,Energy,Acc,0,0);	
